...
sfVector2i *size_matrix)
{
    ...
    for (int p = 0; p < size; p++) {
        ...
        per = (perlin(coord.x, coord.y, 10, perm) + 1) / 2 * 255;
        array[p] = per;
    }
}
...
static void init(water_t *water, framebuffer_t *fb)
{
    ...
    water->p02 = malloc(sizeof(unsigned int) * (fb->width * 2) * fb->height);
    water->shift = 0;
...
static void water_effect(framebuffer_t *fb, water_t *water)
{
    ...
    for (unsigned int p = 0; p < size; p += 4) {
        pix_shift = (p / 2) + water->shift;
        if ((p / 2) - ((p / 2 / fb->width) * fb->width) > fb->width)
            pix_shift = (p / 2) - water->shift;
